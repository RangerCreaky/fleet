...
export default function SecurityPage() {
  return <PageShell eyebrow="Trust" title="Security without pretending risk is zero." intro="Fleet limits what crosses each boundary and keeps Jira separate from local notes.">
    ...
    <h2>Local-only application data</h2><p>Fleet has no backend or external database. Notes and managed attachments are stored in Fleet’s local application data. Writes use a checksummed envelope, atomic promotion, and a rolling backup. Local notes are not transmitted to Jira, GitHub, the Fleet website, or Fleet’s developer.</p>
    <h2>Jira credentials</h2><p>The current beta encrypts Jira connection credentials using Electron safeStorage and keeps them in a separate credential file on the laptop. Credentials are excluded from Fleet note data, note backups, renderer state, and exported diagnostics. They are used only by the Electron main process for requests sent directly to the configured Jira Cloud site.</p>
    ...
    <h2>Open-source verification</h2><p>Fleet’s complete source code is published under the MIT License. Release notes and assets are published through GitHub so users can inspect changes and compare release checksums.</p>
    <h2>Updates and releases</h2><p>Update checks are disabled by default and contact GitHub Releases only when enabled or manually requested. Public production builds are intended to use Apple Developer ID signing, hardened runtime, notarisation, and stapled tickets.</p>
